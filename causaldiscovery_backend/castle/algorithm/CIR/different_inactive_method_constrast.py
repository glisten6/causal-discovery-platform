...
from algorithm.CIR.utils.data_process import get_inactive_candidate
...
if __name__ == "__main__":
    ...
    out_dir = f"algorithm/CIR/exp/inactive_constraint/different_inactive_constraint_method/{m}"
    os.makedirs(out_dir, exist_ok=True)

    alpha = 0.8  # 如果以后想给 inactive 也加传递闭包，这里可以保留

    node_list = [15, 30]
    h_list = [2]  # n_edges = h * n_nodes
    seeds = [3407, 7331, 104729]
    evaluations_name = ["shd", "recall", "precision", "fdr", "fpr", "f1_score"]
    lambda_values = [0.01]  # inactive 的 λ 可以单独扫
    true_ps = [0.4, 0.6]    # 这里直接用作 no_edge_frac
    false_ps = [0.2, 0.0]   # 错误无边先验比例

    # 运行控制
    RUN_EXPERIMENTS = True
    PLOT_FROM_CSV = True

    # inactive 对比配置：只启用 inactive，active 关掉
    config = {
        "active": {
            "use": False,              # 这里关掉 active
            "method": "max",
            "threshold": 0.6,
            "lamb": 0.05,
            "name": "active",
    ...
    "use": True,
    "lamb": 0.01,                    # 初始 λ
    "method": "quad",                # "baseline" / "max" / "quad" / "potential"
    "tau": 0.05,                     # 初始 τ（后面会自适应）
    "beta": 20.0,                    # 初始 β（后面会自适应）
    "sigma": 1e-3,
    "adapt_tau": True,
    "adapt_beta": True,
    "adapt_lamb": False,             # 先关掉 λ 自适应，稳定一点
    ...
    if RUN_EXPERIMENTS:
        for n in [200]:
            ...
            for lambda_val in lambda_values:
                ...
                config["inactive"]["lamb"] = lambda_val

                for n_nodes in node_list:
                    for h in h_list:
                        for no_edge_frac in true_ps:
                            for false_p in false_ps:
                                for seed in seeds:
                                    n_edges = h * n_nodes
                                    print(
                                        f"\n[Inactive] n={n}, n_nodes={n_nodes}, edge={n_nodes * h}, "
                                        f"no_edge_frac={no_edge_frac}, false_p={false_p}, "
                                        f"seed={seed}, lambda={lambda_display}"
                                    )
                                    ...
                                    # ---- 获得无边先验集合（inactive 约束）----
                                    candidate_inactive = get_inactive_candidate(
                                        true_dag,
                                        seed=seed,
                                        false_p=false_p,
                                        no_edge_frac=no_edge_frac,
                                        alpha=alpha,
                                        use_transitive_closure=False,  # 你要求的 False
                                        m=m,
                                    )
                                    ...
                                    base_dir = os.path.join(
                                        lambda_out_dir,
                                        f"n{n_nodes}_h{h}_nefrac{no_edge_frac}_fp{false_p}_seed{seed}"
                                    )
                                    ...
                                    # 1) baseline：L1 无边约束
                                    # ===========================
                                    print("inactive baseline 约束")
                                    config["inactive"]["method"] = "baseline"
                                    al_baseline = NotearsNonlinear(
                                        config=config,
                                    ...
                                    GraphDAG(
                                        al_baseline.causal_matrix,
                                        true_dag,
                                        show=False,
                                        save_name=os.path.join(base_dir, "inactive_baseline.jpg"),
                                    )
                                    met_baseline = MetricsDAG(al_baseline.causal_matrix, true_dag)
                                    print("inactive baseline:", met_baseline.metrics)
                                    ...
                                    results_rows.append({
                                        ...
                                        "f1_score": met_baseline.metrics["F1"],
                                    })

                                    # ===========================
                                    # 2) max：ReLU 风格 |W|>tau 线性惩罚
                                    # ===========================
                                    print("inactive max 约束")
                                    config["inactive"]["method"] = "max"
                                    ...
                                    GraphDAG(
                                        ...
                                        true_dag,
                                        show=False,
                                        save_name=os.path.join(base_dir, "inactive_max.jpg"),
                                    )
                                    met_max = MetricsDAG(al_max.causal_matrix, true_dag)
                                    print("inactive max:", met_max.metrics)
                                    ...
                                    results_rows.append({
                                        ...
                                        "f1_score": met_max.metrics["F1"],
                                    })

                                    # ===========================
                                    # 3) quad：超过 tau 二次增长的平滑 hinge
                                    # ===========================
                                    print("inactive quad 约束")
                                    config["inactive"]["method"] = "quad"
                                    al_quad = NotearsNonlinear(
                                        config=config,
                                        candidate_dict=candidate_dict,
                                        use_pytorch_optimizer=use_pytorch_optimizer,
                                        device_type="cpu",
                                    ...
                                    GraphDAG(
                                        al_quad.causal_matrix,
                                        true_dag,
                                        show=False,
                                        save_name=os.path.join(base_dir, "inactive_quad.jpg"),
                                    )
                                    met_quad = MetricsDAG(al_quad.causal_matrix, true_dag)
                                    print("inactive quad:", met_quad.metrics)
                                    ...
                                    results_rows.append({
                                        ...
                                        "lambda_param": lambda_val,
                                        "method": "quad",
                                        "shd": met_quad.metrics["shd"],
                                        "recall": met_quad.metrics["recall"],
                                        "precision": met_quad.metrics["precision"],
                                        "fdr": met_quad.metrics["fdr"],
                                        "fpr": met_quad.metrics["fpr"],
                                        "f1_score": met_quad.metrics["F1"],
                                    })

                                    # ===========================
                                    # 4) potential：势能型无边约束
                                    # ===========================
                                    print("inactive potential 约束")
                                    config["inactive"]["method"] = "potential"
                                    ...
                                    GraphDAG(
                                        ...
                                        true_dag,
                                        show=False,
                                        save_name=os.path.join(base_dir, "inactive_potential.jpg"),
                                    )
                                    met_potential = MetricsDAG(al_potential.causal_matrix, true_dag)
                                    print("inactive potential:", met_potential.metrics)
                                    ...
                                    results_rows.append({
                                        ...
                                        "f1_score": met_potential.metrics["F1"],
                                    })

                                    # ===========================
                                    # 5) 无约束基线（不传候选）
                                    # ===========================
                                    print("无约束（无 inactive 先验）")
    ...
    # ========================================
    # 二、画图：根据 df 中的 n / false_p / no_edge_frac 画图
    # ========================================
    if RUN_EXPERIMENTS or PLOT_FROM_CSV:
        ...
        for n in n_values:
            ...
            if "lambda_param" not in df.columns:
                fallback_lambda = config.get("inactive", {}).get("lamb", 0.0)
            ...
            # 组合横轴标签（不含 false_p，用于单方法图）
            def make_cfg_base(n_nodes_val, h_val, nefrac_val):
                try:
                    nn = int(n_nodes_val)
                except Exception:
                    nn = n_nodes_val
                try:
                    ...
                    hh = h_val
                return f"n{nn}_h{hh}_ne{nefrac_val}"

            df["config_base"] = df.apply(
                lambda r: make_cfg_base(r["n_nodes"], r["h"], r["no_edge_frac"]), axis=1
            )
            order_base = (
                df[["n_nodes", "h", "no_edge_frac"]]
                .drop_duplicates()
                .sort_values(["n_nodes", "h", "no_edge_frac"])
                .apply(lambda r: f"n{int(r['n_nodes'])}_h{int(r['h'])}_ne{r['no_edge_frac']}", axis=1)
                .tolist()
            )
            ...
            # 含 false_p 的完整横轴
            def make_cfg_full(n_nodes_val, h_val, nefrac_val, false_p_val):
                base = make_cfg_base(n_nodes_val, h_val, nefrac_val)
                fp_str = f"{float(false_p_val):.1f}"
                return f"{base}_fp{fp_str}"

            df["config"] = df.apply(
                lambda r: make_cfg_full(r["n_nodes"], r["h"], r["no_edge_frac"], r["false_p"]),
                axis=1,
            )
            ...
            for lambda_val in lambda_values_in_df:
                ...
                # 每个 inactive 方法，画 false_p 维度的线图
                for method_key, method_title in single_method_list:
                    ...
                    for metric in evaluations_name:
                        ...
                        plt.ylabel(metric)
                        plt.title(
                            f"[Inactive-{method_title}] {metric}（n={n_int}，m={m}，λ={lambda_display}，均值±方差）"
                        )
                        if fp_order_lambda:
                            plt.legend(title="fp")
                        else:
                        ...
                        save_path = os.path.join(
                            n_out_dir,
                            f"lambda_{lambda_tag}_inactive_{method_key}_{metric}_line_by_config.png",
                        )
                ...
                for metric in evaluations_name:
                    ...
                    plt.ylabel(metric)
                    plt.title(
                        f"[Inactive 对比] {metric}（n={n_int}，λ={lambda_display}，均值±方差）"
                    )
                    plt.legend(title="方法")
                    plt.tight_layout()
                    ...
                    cmp_path = os.path.join(
                        n_out_dir,
                        f"lambda_{lambda_tag}_inactive_compare_{metric}_line_by_config.png",
                    )
