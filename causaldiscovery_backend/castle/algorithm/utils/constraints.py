...
class BaseConstraints:
    def penalty(self,model,w: np.ndarray | torch.Tensor) -> float:
...
class ActiveConstraints(BaseConstraints):
    ...
    def penalty(self,model:MLPModel,w: np.ndarray | torch.Tensor,**kwargs) -> float:
        # 获取模型所在设备
...
class OrientationConstraints(BaseConstraints):
    ...
    def penalty(self, model, w: np.ndarray | torch.Tensor,**kwargs) -> float:
        # 获取模型所在设备
        device = next(model.parameters(), None)
        device = device.device if device is not None else torch.device('cpu')
...
class MonoConstraints(BaseConstraints):
    ...
    def penalty(self, model, w_adj: np.ndarray | torch.Tensor, X: torch.Tensor, **kwargs):
