...
class MCSL(BaseLearner):
    ...
    def __init__(self, model_type='nn', num_hidden_layers=4, hidden_dim=16,
        ...
        self.device = device
    def get_mean_effect(self):
        if isinstance(self.data, torch.Tensor):
            x = self.data.clone()
        else:
            # 如果不是 PyTorch 张量，转换为 PyTorch 张量
        ...
        for feature in range(n_features):
            ddx = torch.autograd.grad(self.model.calculate_predict(feature, x).sum(),x,
                                      create_graph=True)[0]

            adj.append(ddx.mean(axis=0).unsqueeze(0))
            
        adj =torch.cat(adj, dim=0).cpu().detach().numpy()
    ...
    def learn(self, data, columns=None, pns_mask=None, **kwargs) -> None:
        ...
        causal_matrix, causal_matrix_weight,model = self._mcsl(x, pns_mask)
    ...
    def _mcsl(self, x, pns_mask) -> tuple:
        ...
        w_est, w_est_weight = callback_after_training(w_logits,
                                                      self.temperature,
                                                      self.graph_thresh)
        return w_est.detach().cpu().numpy(), w_est_weight.detach().cpu().numpy(),model
