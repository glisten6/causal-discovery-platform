...
class DAG_GNN(BaseLearner):
    ...
    def get_mean_effect(self,x):
        ...
        x_pred = self.decoder(logits, adj_A=origin_a, wa=wa)

        x_pred = x_pred.sum(dim=0)
        n_features = x.size()[1]
        adj = []
        for feature in range(n_features):
            ddx = torch.autograd.grad(x_pred[feature], x,
                                      create_graph=True)[0]
            adj.append(ddx.mean(axis=[0,2]).unsqueeze(0))
