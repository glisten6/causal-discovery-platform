...
class GraNDAG(BaseLearner):
    ...
    def get_mean_effect(self,x):
        if isinstance(x, np.ndarray):
            x = torch.Tensor(x).requires_grad_()
        else:
            x = x.requires_grad_()
        weights, biases, extra_params = self.model.get_parameters(mode="wbx")
        y_predict = self.model.compute_log_likelihood(x,weights=weights,biases=biases,extra_params=extra_params).sum(dim = 0)
        n_features = x.shape[1]
        adj = []
        for feature in range(n_features):
            ddx = torch.autograd.grad(y_predict[feature].sum(), x,
                                      create_graph=True)[0]
