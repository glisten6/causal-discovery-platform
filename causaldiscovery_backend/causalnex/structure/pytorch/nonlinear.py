...
class LocallyConnected(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.Tensor(num_linear, input_features, output_features,dtype=torch.float32)
        )
        if bias:
            self.bias = nn.Parameter(torch.Tensor(num_linear, output_features,dtype=torch.float32))
        else:
