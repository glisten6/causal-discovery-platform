...
class NotearsMLP(nn.Module, BaseEstimator):
    ...
    def __init__(
        ...
        self.dag_layer = nn.Linear(
            self.dims[0], self.dims[0] * self.dims[1], bias=use_bias,dtype=torch.float32,
        )
        self.dag_layer.to(torch.float32)
    ...
    def reconstruct_data(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            return np.asarray(X_hat.cpu().detach().numpy().astype(np.float32))
    ...
    def _dual_ascent_step(
        ...
        def _func(flat_params: np.ndarray) -> Tuple[float, np.ndarray]:
            ...
            return loss, flat_grad.astype("float32")
