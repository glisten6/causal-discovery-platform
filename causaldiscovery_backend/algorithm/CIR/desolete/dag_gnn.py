...
from gcastle.trustworthyAI.gcastle.castle.common import BaseLearner, Tensor
from gcastle.trustworthyAI.gcastle.castle.common import consts
from gcastle.trustworthyAI.gcastle.castle.common.validator import check_args_value

from CIR.utils import functions as func
from CIR.models import models_alter as md
from  CIR.models.models_alter  import Encoder, Decoder
...
class DAG_GNN(BaseLearner):
    ...
    def __init__(self, encoder_type='mlp', decoder_type='mlp',
                 ...
                 encoder_dropout=0.0, decoder_dropout=0.0, epochs=300, k_max_iter=1e2, tau_a=0.05,
    ...
    def _train(self, train_loader, optimizer, lambda_a, c_a):
        ...
        for batch_idx, (data, relations) in enumerate(train_loader):
            ...
            mu,log_vari, origin_a = self.encoder(x)

            z_gap = self.encoder.z
            z_positive = self.encoder.z_positive
            wa = self.encoder.wa
            z = self.encoder.reparameterization(mu,log_vari)

            # log_vari = torch.zeros_like(mu)
            # z = self.encoder.reparameterization(mu,log_vari)


            x_pred = self.decoder(z, adj_A=origin_a, wa=wa)    # X_hat
            # reconstruction accuracy loss
            loss_nll = func.nll_gaussian(x_pred, x)

            # KL loss
            loss_kl = func.kl_gaussian_sem(mu,log_vari)
