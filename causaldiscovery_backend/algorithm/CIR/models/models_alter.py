...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, adj_A, device=None, encoder_type='mlp'):
        ...
        if self.encoder_type == 'mlp':
            self.mlp_mu = MLP(input_dim, hidden_dim, output_dim, device=device)
            self.mlp_vari = MLP(input_dim, hidden_dim, output_dim, device=device)
    ...
    def forward(self, x):
        ...
        if self.encoder_type == 'mlp':
            mlp_out_mu = self.mlp_mu(x.to(self.mlp_mu.device))
            mu = torch.matmul(adj_Aforz, mlp_out_mu + self.wa) - self.wa

            mlp_out_vari = self.mlp_vari(x.to(self.mlp_vari.device))
            vari = torch.matmul(adj_Aforz, mlp_out_vari + self.wa) - self.wa


        return mu,vari , adj_A1
