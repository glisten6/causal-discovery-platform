...
import numpy as np
from CIR.utils.locally_connected import LocallyConnected
from castle.algorithms import NotearsNonlinear
from castle.algorithms.gradient.notears.torch.models import Notear
...
class MLPModel(nn.Module):
    def __init__(self, dims, bias=True, device=None):
        ...
        self.fc = nn.Linear(d,d *dims[1],bias = bias,device = self.device)
    ...
    def h_func(self):
        ...
        """
        d = self.dims[0]
        fc1_weight = self.fc.weight  # [j * m1, i]
    ...
    def cir(self,cir_regulation):
        if isinstance(cir_regulation,np.ndarray):
            cir_regulation = torch.from_numpy(cir_regulation)

        fc1_weight = self.fc.weight
        fc1_weight = fc1_weight.view(self.dims[0],-1,self.dims[0])
        w = torch.sum(fc1_weight * fc1_weight,dim = 1).t()

        reg = torch.mul(cir_regulation,w)
        return torch.sum(reg ** 2)
    ...
    def fc1_l1_reg(self):
        ...
        """
        reg = torch.norm(self.fc.weight,p = 1)
        return reg
