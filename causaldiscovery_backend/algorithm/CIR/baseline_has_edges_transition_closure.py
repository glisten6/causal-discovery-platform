...
if __name__ == "__main__":
    ...
    m = 4
    sem_type = 'gp-add'
    out_dir = f"algorithm/CIR/exp/pytorch_optimizer/active_constraint/lambda/{m}"
    ...
    lambda_values = [ 0.1, 0.2,0.5]  # 可按需调整不同的约束强度
                        ...
                        for true_p in true_ps:
                            for false_p in [0.2, 0.0]:
                                for seed in seeds:
                                    ...
                                    # ===========================
                                    print("无约束")
                                    al_none = NotearsNonlinear(device_type="cpu",use_pytorch_optimizer=use_pytorch_optimizer)
                                    al_none.learn(X)
    ...
    if RUN_EXPERIMENTS or PLOT_FROM_CSV:
        ...
        for n in n_values:
            ...
            method_map = {
                "with_constraint_adj": "有约束（邻接）",
                "with_constraint_tc": "有约束（传递闭包）",
                "no_constraint": "无约束",
            }
            ...
            for lambda_val in lambda_values_in_df:
                ...
                for method_key, method_title in single_method_list:
                    ...
                    for metric in evaluations_name:
                        ...
                        plt.ylabel(metric)
                        plt.title(
                            f"{method_title}：{metric}（n={n_int}，m={m}，λ={lambda_display}，均值±方差）"
                        )
                        if fp_order_lambda:
                            plt.legend(title="fp")
                        else:
